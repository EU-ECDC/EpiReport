...
* A complete ECDC Atlas export (csv format from the ECDC Atlas restricted access) 
or any dataset specified as described below.
...
If the user wishes to set different parameters for one of the 53 covered diseases, 
...
* `TableUse`: Character string, specifying whether to include the table in the AER report and which table to include:
...
bar graph in the AER report and which type of graph to include:
...
of the geographical code used `GeoCode` in the disease dataset, 
...
To generate an Annual Epidemiological Report, we can use the default dataset 
included in the `EpiReport` package presenting Salmonellosis data for 2016.
...
folder) is used to produce the corresponding AER report.
...
However, it is optional as the user can prefer to generate the plots with the default
`Arial` font. In that case, warnings will appear in the console for each plot.



### 2.2. Word template

By default, an empty ECDC template (Microsoft Word) is used to produce the AER report. 
...
Word template when producing the AER report as described below.
...
## 3. How to generate each AER outputs independently

The `EpiReport` package allows the user to generate each AER output 
...
* __Seasonal plot__: Distribution of cases at EU/EEA level, by month,
over the past five years.
* __Trend plot__: Trend and number of cases at EU/EEA level, by month,
over the past five years.
* __Map__: Distribution of cases by country presenting either:
    + the number of cases;
...
By default, the function will use the internal Salmonellosis 2016 data and
...
### 3.2. Seasonal plot: distribution of cases by month
...
By default, the function will use the internal Salmonellosis 2016 data.
...
In the example below, we use Pertussis 2016 data.
...
* The moving average of the number of cases by month (green solid line)
...
In the example below, we use again Pertussis 2016 data.
...
By default, the function will use the internal Salmonellosis 2016 data with 
...
In the example below, we use Zika 2016 data.
